...
class CustomFormatBundle3D(DefaultFormatBundle):
    ...
    def __call__(self, results):
        ...
        for key in self.collect_keys + ['voxel_semantics', 'mask_camera']:
            ...
            if key in ['timestamp', 'img_timestamp']:
                results[key] = DC(to_tensor(np.array(results[key], dtype=np.float64)), cpu_only=False, stack=True, pad_dims=None)
            else:
                results[key] = DC(to_tensor(np.array(results[key], dtype=np.float32)), cpu_only=False, stack=True, pad_dims=None)

        for key in ['voxels', 'coors', 'voxel_centers', 'num_points', 'gt_depths']:
            if key not in results:
                continue
...
class Collect3D(object):
    ...
    def __init__(
        ...
        meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                   ...
                   'start_of_sequence', 'ego2global', 'scene_name',
